...
out <- left_join(new_fam, old_fam, by = "FIDIID") %>%
...
message('\nWriting .fam file: ', snakemake@output[["fixed"]])
out %>%
  write_tsv(snakemake@output[["fixed"]], col_names = F)

# .Pop file for admixture
ref <- left_join(
  snakemake@input[['ref_pops']] %>% read_table2() %>% rename(pops = Population),
  snakemake@input[['ref_superpops']] %>% read_table2() %>% rename(super_pop = Population)
)

message('\nWriting .pop file: ', snakemake@output[["pops"]])
left_join(out, ref) %>%
  mutate(super_pop = ifelse(is.na(super_pop), "-", super_pop)) %>%
  select(super_pop) %>%
  write_tsv(snakemake@output[["pops"]], col_names = F)
